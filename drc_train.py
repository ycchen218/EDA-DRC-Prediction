...
def train(rootpath,batch_size,num_epochs,lr,fig_savepath,weight_savepath):
    ...
    #data
    dataset = DRCDataset(root_dir=rootpath,transform=True)
    ...
    model = DRCModel(device=device).to(device)
    ...
    for e in tqdm(range(num_epochs), desc='Epoch'):
        ...
        if v/n2 < best_test_Loss and e>100:
            ...
            torch.save(model.state_dict(), f'{weight_savepath}/drc_weights.pt')
            best_test_Loss = v/n2
        if t/n1 < best_train_Loss:
            print(f'Best Epoch {e}: Train Loss: {t/n1}')
            torch.save(model.state_dict(), f'{weight_savepath}/drc_train_weights.pt')
    ...
    def parse_args():
        ...
        parser.add_argument("--root_path", default="./CircuitNet/dataset/DRC", type=str,
        ...
        parser.add_argument("--weight_path", default="./model_weight", type=str,
                            help='The path to save the model weight')
