...
class FPN(nn.Module):
    def __init__(self, block, num_blocks, device):
        ...
        self.in_planes = 64
        if device=='cpu':
            self.conv1 = CoordConv2d(9, 64, kernel_size=3, stride=1, padding=1, bias=False,use_cuda=False)
        else:
            self.conv1 = CoordConv2d(9, 64, kernel_size=3, stride=1, padding=1, bias=False)
